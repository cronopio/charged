...
Charged.prototype.getCustomerById = function(id, callback) {
  return this.get('/customers/' + id, callback, 'customer');
};
...
Charged.prototype.deleteCustomerById = function(id, callback) {
  return this.delete('/customers/' + id, callback, 'customer');
};
...
Charged.prototype.updateCustomerById = function(id, options, callback) {
  return this.put('/customers/' + id, options, callback, 'customer');
};
...
Charged.prototype.getCustomerSubscriptions = function(id, callback) {
  return this.get('/customers/' + id + '/subscriptions', callback, 'subscription');
};
...
Charged.prototype.getSubscriptionByRef = function(name, callback) {
  return this.getSubscriptions(function(err, sub) {
    if (err) return callback(err);
    var l = sub.length
      , i = 0;

    for (; i < l; i++) {
      if (sub[i].customer.reference === name) {
        return callback(null, sub[i]);
      }
    }
...
Charged.prototype.listProductFamilyComponents = function(id, callback) {
  // XXX unsure of the formatter
  var path = '/product_families/' + id + '/components';
  return this.get(path, callback, 'component');
};
...
Charged.prototype.request = function(options, callback) {
  ...
  return request(options, function(err, res, body) {
    if (res.statusCode === 403) return callback(new Error('Not supported.'));
    try {
